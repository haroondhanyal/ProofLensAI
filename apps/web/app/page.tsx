...
import { ShieldCheck, Link2, MessageSquareText, Image as ImageIcon, QrCode, FileSearch, History, LayoutDashboard, Settings, ChevronRight, ArrowUpRight, LoaderCircle, AlertTriangle, CheckCircle2, LockKeyhole, ScanLine, Eye, EyeOff, Camera, UserRound, FileText, Radar, Bookmark, ArrowRight, CircleHelp, Shield } from 'lucide-react';

const API = process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:8000/api/v1';
const NAV_ITEMS = [{label:'Overview',Icon:LayoutDashboard},{label:'Analyze',Icon:ScanLine},{label:'History',Icon:History},{label:'Reports',Icon:FileText},{label:'Threat Center',Icon:Radar},{label:'Saved',Icon:Bookmark}];
...
const THREAT_GUIDES = [
  {title:'Phishing',tag:'LINKS & LOGIN PAGES',copy:'Fake sign-in pages imitate a service and ask for passwords, payment details, or one-time codes.',signs:'Unexpected sign-in links, look-alike domains, urgent account warnings.',action:'Open the service through its official app or a saved bookmark.'},
  {title:'Fake delivery notices',tag:'SMS & EMAIL',copy:'A message claims a parcel is delayed and asks for a small fee or personal details.',signs:'Unfamiliar tracking links, unexpected customs fees, requests for card details.',action:'Check tracking in the courier’s official app using a number you already have.'},
  {title:'Bank impersonation',tag:'CALLS & MESSAGES',copy:'A sender pretends to be a bank and pressures you to move money or share a code.',signs:'OTP requests, urgent transfers, remote-access app instructions.',action:'Hang up and call the number printed on your card.'},
  {title:'Account takeover',tag:'PASSWORD & OTP',copy:'Attackers use fake security alerts to capture your password or recovery code.',signs:'Unexpected reset prompts, login codes you did not request, threats of closure.',action:'Never share a code; change your password from the official app.'},
  {title:'Fake prizes',tag:'GIVEAWAYS',copy:'A prize message asks for a fee, identity document, or payment details before you can claim it.',signs:'You did not enter, a short deadline, an advance payment request.',action:'Verify the promotion on the organization’s official website.'},
  {title:'Investment scams',tag:'MONEY & CRYPTO',copy:'A promoter promises unusually high or guaranteed returns to encourage a quick deposit.',signs:'Guaranteed profits, private chat groups, pressure to recruit others.',action:'Pause and check the provider with your local financial regulator.'},
  {title:'Marketplace fraud',tag:'BUYING & SELLING',copy:'A buyer or seller moves the conversation off-platform or sends a payment/delivery link.',signs:'Overpayment, fake escrow, pressure to pay outside the platform.',action:'Keep messages and payments inside the platform’s official flow.'},
  {title:'QR code traps',tag:'QR & SHORT LINKS',copy:'A QR code can hide a destination that is hard to inspect before opening.',signs:'Stickers placed over a code, unexpected login or payment pages.',action:'Preview the decoded address and verify the domain before opening it.'},
  {title:'Tech support fraud',tag:'POP-UPS & CALLS',copy:'A fake warning claims your device is infected and asks you to call or install remote-access software.',signs:'Browser lock screens, unsolicited support calls, requests for remote access.',action:'Close the page and contact the device maker through its official support site.'},
  {title:'Romance & friendship fraud',tag:'SOCIAL & MESSAGING',copy:'A new contact builds trust, then asks for money, gift cards, or help with an emergency.',signs:'Avoiding in-person/video verification, urgent financial requests, secrecy.',action:'Verify independently and talk to someone you trust before sending money.'},
];
...
export default function Home() {
  ...
  function openAuthScreen(mode: 'login' | 'register', destination: 'Overview' | 'Analyze' = 'Overview', analysisMode: 'url' | 'message' | 'screenshot' | 'qr' | 'image' | 'file' = 'url') {
    setAuthMode(mode);
    setPage(destination);
  ...
  useEffect(() => {
    if (!token || demoMode || !['History', 'Reports', 'Saved'].includes(page)) return;
    const timer = setTimeout(async () => {
      const params = new URLSearchParams({limit:'50'});
      if (scanTypeFilter) params.set('scan_type', scanTypeFilter);
      if (riskFilter) params.set('risk_level', riskFilter);
      if (page === 'Saved' || savedOnly) params.set('saved_only', 'true');
  ...
  }, [token,demoMode,page,scanTypeFilter,riskFilter,savedOnly,historyQuery,dateFrom,dateTo]);
  ...
  async function removeAccount() {
    setActionError('');
    try { const response=await apiFetch(`${API}/auth/me`,{method:'DELETE',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:accountPassword})}); const body=await response.json(); if(!response.ok)throw new Error(body?.error?.message||'Account could not be deleted.'); setToken(''); setShowLanding(true); setScans([]); }
  ...
  const threatCenterContent = <section className="threat-center-page"><div className="threat-page-heading"><div><span className="hero-kicker">THREAT CENTER · PRACTICAL GUIDES</span><h1>Common ways people get targeted</h1><p>Educational guidance only. These cards are not live threat alerts or provider findings.</p></div><div className="threat-page-mark"><Radar size={25}/><span>STAY INFORMED</span></div></div><div className="threat-guide-grid">{THREAT_GUIDES.map((guide,index)=><article className="threat-guide-card" key={guide.title}><div className="threat-card-top"><span>0{index+1}</span><small>{guide.tag}</small></div><h2>{guide.title}</h2><p>{guide.copy}</p><div className="threat-detail"><b>Warning signs</b><span>{guide.signs}</span></div><div className="threat-detail action"><b>Safer next step</b><span>{guide.action}</span></div></article>)}</div><aside className="threat-disclaimer"><LockKeyhole size={17}/><span><b>No live threat feed is connected to this guide.</b> Check the scan report for provider availability and sources related to a specific item.</span><button onClick={()=>setPage('Analyze')}>Check something <ArrowRight size={15}/></button></aside></section>;
  const collectionTitle = page === 'Saved' ? 'Saved checks' : page === 'Reports' ? 'Your reports' : 'Scan history';
  const collectionItems = page === 'Saved' ? scans.filter(scan=>scan.is_saved) : scans;
  const collectionContent = <section className="collection-page"><div className="collection-heading"><div><span className="hero-kicker">YOUR PRIVATE WORKSPACE</span><h1>{collectionTitle}</h1><p>{page==='Saved'?'Checks you marked to revisit.':page==='Reports'?'Open a past assessment, download it, or create a private share link.':'Search and filter your past checks.'}</p></div><button className="collection-primary" onClick={()=>setPage('Analyze')}><ScanLine size={16}/> New check</button></div><div className="collection-toolbar"><label className="collection-search"><History size={16}/><input aria-label="Search checks" placeholder="Search your checks…" value={historyQuery} onChange={e=>setHistoryQuery(e.target.value)}/></label><select aria-label="Filter by check type" value={scanTypeFilter} onChange={e=>setScanTypeFilter(e.target.value)}><option value="">All check types</option>{['URL','MESSAGE','SCREENSHOT','QR','IMAGE','FILE','STORE','PRODUCT','CLAIM'].map(type=><option key={type}>{type}</option>)}</select><select aria-label="Filter by risk" value={riskFilter} onChange={e=>setRiskFilter(e.target.value)}><option value="">All risk levels</option>{['LOW','CAUTION','HIGH','CRITICAL'].map(level=><option key={level}>{level}</option>)}</select><label className="date-filter">From<input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)}/></label><label className="date-filter">To<input type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)}/></label></div>{collectionItems.length===0?<div className="collection-empty"><span><Bookmark size={20}/></span><h2>{page==='Saved'?'Nothing saved yet':page==='History'?(scans.length?'No checks match these filters':'No checks yet'):'No reports yet'}</h2><p>{page==='Saved'?'Save a check from its report to keep it here.':page==='History'?'Adjust the filters or run a new check to see it here.':'Run a check and its evidence report will appear here.'}</p><button onClick={()=>setPage('Analyze')}>Start a check <ArrowRight size={15}/></button></div>:<div className="collection-list">{collectionItems.map(scan=><article className="collection-row" key={scan.scan_id}><div className={`collection-risk ${scan.risk_level.toLowerCase()}`}><span>{scan.risk_score}</span><small>/100</small></div><div className="collection-main"><div className="collection-meta"><span className={`row-badge ${scan.risk_level.toLowerCase()}`}>{scan.risk_level}</span><span>{scan.scan_type}</span><span>{scan.created_at?new Date(scan.created_at).toLocaleDateString():''}</span></div><h2>{scan.scan_type} assessment</h2><p>{scan.summary}</p><small>{scan.scan_id} · {scan.confidence} confidence · {scan.evidence.length} evidence signals</small></div><div className="collection-actions"><button className="collection-open" onClick={()=>{setResult(scan);setPage('Analyze')}}>Open report <ArrowRight size={14}/></button>{!scan.is_demo&&<button className="collection-save" onClick={()=>void scanAction('save',scan.scan_id)}>{scan.is_saved?'Remove saved':'Save check'}</button>}</div></article>)}</div>}</section>;
  const helpContent = <section className="info-page"><span className="hero-kicker">HELP CENTER</span><h1>Use ProofLens with care.</h1><p className="info-intro">A quick guide to running a check and understanding its limits.</p><div className="info-card-grid"><article><span>01 · START</span><h2>Choose the right check</h2><p>Use URL for a web address, Message for copied text, Screenshot or Image for an image file, QR for a QR image, and File for a supported document or archive.</p><button onClick={()=>setPage('Analyze')}>Open analyzer <ArrowRight size={14}/></button></article><article><span>02 · REVIEW</span><h2>Read the evidence</h2><p>Check the report summary, evidence sources, confidence, provider status, and any note that the analysis was partial.</p><button onClick={()=>setPage('Reports')}>Open reports <ArrowRight size={14}/></button></article><article><span>03 · ACT</span><h2>Use official channels</h2><p>Do not share passwords or verification codes. Verify urgent payment requests using a phone number or app you already trust.</p><button onClick={()=>setPage('Threat Center')}>Browse safety guides <ArrowRight size={14}/></button></article></div><div className="help-faq"><h2>Common questions</h2><details><summary>Does LOW risk mean the link is safe?</summary><p>No. It means the available checks found fewer warning signals. New or unknown threats can be missed.</p></details><details><summary>Why does a report say analysis was limited?</summary><p>A scanner or optional provider may be unavailable, or the available evidence may not support a stronger conclusion.</p></details><details><summary>Can I share a report?</summary><p>Yes. Use the explicit private-report action on a live report. Shared reports omit the original submitted content, and you can revoke the link.</p></details></div></section>;
  const privacyContent = <section className="info-page"><span className="hero-kicker">PRIVACY & DATA</span><h1>Know what happens to a check.</h1><p className="info-intro">ProofLens processes content to create the analysis you request. Your configured API and optional providers determine where submitted data is handled.</p><div className="privacy-flow-list"><article><span>01</span><div><h2>When you start a scan</h2><p>The text, URL, or selected file is sent to the configured ProofLens API. URL page fetching is opt-in. Optional provider services may receive the data needed for the enabled check.</p></div></article><article><span>02</span><div><h2>Reports and history</h2><p>Completed scans appear in your private account history. You can delete an individual scan from its report or the reports list.</p></div></article><article><span>03</span><div><h2>When you share</h2><p>A report becomes shareable only after you explicitly create a link. Shared reports leave out the original submitted content. Revoke a link from the report actions.</p></div></article><article><span>04</span><div><h2>Account controls</h2><p>Update your profile, change your password, choose a local display theme, or permanently delete your account and its data from Settings.</p></div></article></div><div className="privacy-page-actions"><button onClick={()=>setPage('Settings')}><Settings size={15}/> Open account settings</button><span>For local database retention and provider settings, ask the administrator of this ProofLens deployment.</span></div></section>;
  if (!token && showLanding) return <main className="marketing-site">
    <header className="marketing-nav">
      <a className="brand marketing-brand" href="#top"><span className="brand-icon"><ShieldCheck size={21}/></span><span>ProofLens<span className="brand-ai">AI</span></span></a>
      <nav aria-label="Public navigation"><a href="#checks">What you can check</a><a href="#how-it-works">How it works</a><a href="#faq">FAQ</a></nav>
      <div className="marketing-nav-actions"><button className="marketing-signin" onClick={()=>openAuthScreen('login')}>Sign in</button><button className="marketing-nav-cta" onClick={()=>openAuthScreen('register')}>Create account <ArrowRight size={15}/></button></div>
    </header>
    <section className="marketing-hero" id="top">
      <div className="hero-copy"><span className="hero-kicker"><span/> DIGITAL TRUST, MADE CLEAR</span><h1>Check before<br/>you <em>trust.</em></h1><p>Pause on suspicious links, messages, screenshots, QR codes, and files. Get clear signals and practical next steps before you click, pay, or share.</p><div className="hero-actions"><button className="hero-primary" onClick={()=>openAuthScreen('register','Analyze')}>Analyze something <ArrowRight size={17}/></button><button className="hero-secondary" onClick={openDemo}>Explore sample reports <ArrowUpRight size={16}/></button></div><div className="hero-trust"><span><CheckCircle2 size={15}/> Evidence explained</span><span><LockKeyhole size={14}/> Private workspace</span></div></div>
      <div className="hero-visual" aria-label="How a ProofLens check is reviewed"><div className="visual-orbit visual-orbit-one"/><div className="visual-orbit visual-orbit-two"/><div className="review-card"><div className="review-card-top"><span className="review-logo"><ShieldCheck size={16}/></span><span><b>ProofLens check</b><small>Evidence-first review</small></span><span className="review-live"><i/> READY</span></div><div className="review-content"><span className="review-label">YOUR CONTENT STAYS IN YOUR CONTROL</span><h2>See the signals.<br/>Choose what to do next.</h2><div className="review-steps"><div><span>01</span><p><b>Submit</b><small>Choose the item you want to check</small></p></div><div><span>02</span><p><b>Review evidence</b><small>See available findings and limits</small></p></div><div><span>03</span><p><b>Make your choice</b><small>Use the recommended safe next step</small></p></div></div><div className="review-foot"><LockKeyhole size={14}/> Analysis starts when you choose it</div></div></div></div>
    </section>
    <section className="landing-checks" id="checks"><div className="section-heading"><div><span className="hero-kicker">ONE WORKSPACE, MANY CHECKS</span><h2>What would you like to verify?</h2></div><p>Start with the content in front of you. ProofLens shows what it could inspect and where it had limits.</p></div><div className="landing-check-grid">{[{Icon:Link2,label:'Website links',mode:'url' as const,copy:'Inspect a URL and optionally review public redirects and forms.'},{Icon:MessageSquareText,label:'Messages',mode:'message' as const,copy:'Review suspicious SMS, email, and chat wording.'},{Icon:ImageIcon,label:'Screenshots',mode:'screenshot' as const,copy:'Extract visible text and look for warning signals.'},{Icon:QrCode,label:'QR codes',mode:'qr' as const,copy:'Decode a destination before deciding to open it.'},{Icon:FileSearch,label:'Images',mode:'image' as const,copy:'Review metadata and available image signals.'},{Icon:FileText,label:'Files',mode:'file' as const,copy:'Inspect supported documents and archives without executing them.'}].map(({Icon,label,mode:checkMode,copy},index)=><button className="landing-check-card" key={label} onClick={()=>openAuthScreen('register','Analyze',checkMode)}><span className="landing-check-icon"><Icon size={19}/></span><span className="landing-card-index">0{index+1}</span><b>{label}</b><small>{copy}</small><span className="landing-card-link">Check this <ArrowRight size={14}/></span></button>)}</div></section>
    <section className="how-section" id="how-it-works"><div className="section-heading"><div><span className="hero-kicker">HOW PROOFLENS WORKS</span><h2>From doubt to a clearer next step.</h2></div><p>No instant verdict from a black box. Review the available evidence and understand what remains unknown.</p></div><div className="how-grid"><article><span>01</span><h3>Submit</h3><p>Paste, upload, or share the item you want to review.</p></article><article><span>02</span><h3>Analyze</h3><p>Available rules and configured providers check relevant signals.</p></article><article><span>03</span><h3>Verify</h3><p>See where each signal came from and whether a provider was available.</p></article><article><span>04</span><h3>Understand</h3><p>Read a plain-language summary and practical recommended actions.</p></article></div></section>
    <section className="landing-capabilities" id="capabilities"><div className="section-heading"><div><span className="hero-kicker">DESIGNED TO EXPLAIN</span><h2>Safety tools that show their work.</h2></div></div><div className="capability-grid"><article><ShieldCheck/><h3>Scam detection</h3><p>Spot common warning signs in messages, links, stores, and claims.</p></article><article><Link2/><h3>Website safety</h3><p>Inspect public URL signals and opt into bounded page checks.</p></article><article><ImageIcon/><h3>Screenshot analysis</h3><p>Review extracted text while seeing whether OCR was available.</p></article><article><Radar/><h3>Media signals</h3><p>Show authenticity-provider findings only when configured, with limits clearly stated.</p></article><article><QrCode/><h3>QR safety</h3><p>Preview a decoded destination and its available link signals.</p></article><article><LockKeyhole/><h3>Privacy controls</h3><p>Keep an account history, delete checks, and explicitly choose when to share a report.</p></article></div></section>
    <section className="landing-privacy" id="privacy"><div className="privacy-mark"><LockKeyhole size={25}/></div><div><span className="hero-kicker">YOUR DATA, YOUR DECISION</span><h2>Nothing gets a safety guarantee.</h2><p>ProofLens explains what was checked and what was unavailable. Submitted material is sent to your configured ProofLens API when you start an analysis. Optional external providers may receive the data needed for that check.</p></div><a className="hero-secondary" href="#faq">Privacy questions <ArrowRight size={15}/></a></section>
    <section className="landing-faq" id="faq"><div className="section-heading"><div><span className="hero-kicker">GOOD TO KNOW</span><h2>Frequently asked questions</h2></div></div><div className="faq-list"><details><summary>Does a low-risk result guarantee something is safe?</summary><p>No. A result reflects the evidence available at that time. A low-risk score is not a guarantee; verify important requests through official channels.</p></details><details><summary>Does ProofLens send every item to an AI model?</summary><p>No. Deterministic checks and configured providers make up the assessment. Optional local AI can explain some message findings, but it does not set the risk score.</p></details><details><summary>Do I need provider keys to explore the app?</summary><p>No. You can explore clearly labeled fictional sample reports without a live backend. Live checks use your configured API and only the integrations enabled there.</p></details></div></section>
    <section className="landing-cta"><div><span className="hero-kicker">NOT SURE ABOUT SOMETHING?</span><h2>Take a moment. Check it with ProofLens.</h2><p>Start with a link, message, image, QR code, or supported file.</p></div><button className="hero-primary" onClick={()=>openAuthScreen('register','Analyze')}>Check something now <ArrowRight size={17}/></button></section>
    <footer className="marketing-footer"><a className="brand marketing-brand" href="#top"><span className="brand-icon"><ShieldCheck size={18}/></span><span>ProofLens<span className="brand-ai">AI</span></span></a><span>Evidence, not certainty. Verify important requests independently.</span><div><a href="#privacy">Privacy</a><a href="#faq">Help & FAQ</a><button onClick={()=>openAuthScreen('login')}>Account</button></div></footer>
  </main>;
  if (!token) return <main className="auth-screen"><div className="auth-layout">
    ...
    <section className="auth-card"><div className="auth-mobile-brand"><span className="brand-icon"><ShieldCheck size={20}/></span><span>ProofLens<span className="brand-ai">AI</span></span></div><button className="auth-back-home" onClick={()=>setShowLanding(true)}>← Back to ProofLens home</button><div className="eyebrow">{forgotMode?'ACCOUNT RECOVERY':authMode==='register'?'GET STARTED':'WELCOME BACK'}</div><h1>{forgotMode?'Reset your password':authMode==='register'?'Create your workspace':'Welcome back'}</h1><p className="auth-intro">{forgotMode?'Enter your account email to get a secure one-time reset link.':authMode==='register'?'Create an account for private scans and a personal safety history.':'Sign in to access your private checks and scan history.'}</p>
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <div className="side-bottom"><div className="privacy-card"><LockKeyhole size={17}/><div><b>Private by design</b><small>Your checks stay yours.</small></div></div><button className={`nav-item ${page==='Help'?'selected':''}`} onClick={()=>setPage('Help')}><CircleHelp size={17}/>Help</button><button className={`nav-item ${page==='Privacy'?'selected':''}`} onClick={()=>setPage('Privacy')}><Shield size={17}/>Privacy</button>{!demoMode&&<button className={`nav-item ${page==='Settings'?'selected':''}`} onClick={()=>setPage('Settings')}><Settings size={17}/>Account & settings</button>}<button className="profile profile-button" onClick={async()=>{if(!demoMode)await apiFetch(`${API}/auth/logout`,{method:'POST',credentials:'include'}).catch(()=>{});setDemoMode(false);setProfile(null);setToken('');setShowLanding(true);setScans([]);setResult(null);}} title="Sign out"><div className="avatar">{avatarSource?<img src={avatarSource} alt=""/>:profileInitial}</div><div><b>{demoMode?'Sample workspace':profile?.display_name||profile?.email||'Account'}</b><small>{demoMode?'Exit sample':'Sign out'}</small></div><ChevronRight size={15}/></button></div>
    </aside>

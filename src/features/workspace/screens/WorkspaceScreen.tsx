...
import { apiFetch, apiUrl } from '../../../core/api/client';
...
export default function App() {
  ...
  useEffect(()=>{ apiFetch('/auth/me').then(async r=>{if(r.ok){const body=await r.json();const me=body.data as UserProfile;setProfile(me);setProfileName(me.display_name||'');const parsed=splitPhone(me.phone);setProfileCountry(parsed.country);setProfilePhone(parsed.number);setUser(me.display_name||me.email);loadHistory();}}).catch(()=>{}); },[]);
  ...
  async function authenticate(){
    ...
    try {if(authMode==='register'&&(password.length<10||password!==confirmPassword))throw new Error(password.length<10?'Password kam az kam 10 characters ka ho.':'Dono passwords match nahi karte.');const endpoint=authMode==='register'?'/auth/register':'/auth/login';const data=authMode==='register'?{email,password,display_name:name,phone:phone?`${phoneCountry.dial}${phone}`:null}:{email,password};const response=await apiFetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});const body=await response.json();if(!response.ok)throw new Error(body?.error?.message||body?.detail||'Sign in nahi ho saka.');const me:UserProfile={email:body.data.user.email,display_name:body.data.user.display_name,phone:body.data.user.phone};const parsed=splitPhone(me.phone);setProfile(me);setProfileName(me.display_name);setProfileCountry(parsed.country);setProfilePhone(parsed.number);setUser(me.display_name||me.email);if(authMode==='register'&&profilePhoto){try{await uploadProfilePhoto(profilePhoto)}catch(reason){Alert.alert('Profile photo upload nahi hui',reason instanceof Error?reason.message:'Settings mein dobara koshish karein.')}}loadHistory();}
  ...
  async function shareReport(scan:Scan){
    ...
    try {const response=await apiFetch(`/reports/${scan.scan_id}/share`,{method:'POST'});const body=await response.json();if(!response.ok)throw new Error(body?.error?.message||'Share link nahi bana.');await Share.share({message:`ProofLens report: ${body.data.url}`});}catch(e){Alert.alert('Share nahi ho saka',e instanceof Error?e.message:'Try again.');}
  }
  ...
  async function signOut(){if(!demo)await apiFetch('/auth/logout',{method:'POST'}).catch(()=>{});setUser('');setScans([]);setSelected(null);setDemo(false);}
  function startDemo(){setDemo(true);setUser('Sample workspace');setScans(DEMO_SCANS);setSelected(null);setTab('History');}

  const palette = { Light: { bg:'#f7f9fa',surface:'#ffffff',text:'#192b3d',accent:'#286c8b',border:'#e7ecee' }, Ocean: { bg:'#eaf5f7',surface:'#f4fbfc',text:'#17354a',accent:'#116f83',border:'#bdd9df' }, Dark: { bg:'#111820',surface:'#1c2731',text:'#edf2f7',accent:'#75c9e7',border:'#3b4652' }, Gray: { bg:'#242a33',surface:'#303844',text:'#e5ebf1',accent:'#a5c9e7',border:'#49515e' }, 'High contrast': { bg:'#ffffff',surface:'#ffffff',text:'#000000',accent:'#0037a5',border:'#111111' } }[theme];
  ...
  return <View style={[styles.screen,{backgroundColor:palette.bg}]}><StatusBar style={theme==='Dark'||theme==='Gray'?'light':'dark'}/><View style={[styles.header,{backgroundColor:palette.surface,borderBottomColor:palette.border}]}><View><Text style={[styles.brand,{color:palette.text}]}>ProofLens <Text style={styles.ai}>AI</Text></Text><Text style={[styles.headerSub,{color:palette.text}]}>Local analysis · {user}</Text></View><View style={styles.headerActions}>{!demo&&<Pressable style={styles.headerProfile} onPress={()=>setTab('Settings')}>{profile?.avatar_url?<Image source={{uri:`${apiUrl('/auth/me/avatar')}?v=${avatarVersion}`}} style={styles.headerAvatar}/>:<View style={styles.headerAvatar}><Text style={styles.avatarInitial}>{(profile?.display_name||user).slice(0,1).toUpperCase()}</Text></View>}<Text style={[styles.linkText,{color:palette.accent}]}>{profile?.display_name||user}</Text></Pressable>}<Pressable onPress={signOut}><Text style={styles.linkText}>{demo?'Exit demo':'Sign out'}</Text></Pressable></View></View>
    ...
    {cameraOpen?<View style={styles.camera}><CameraView style={StyleSheet.absoluteFill} barcodeScannerSettings={{barcodeTypes:['qr']}} onBarcodeScanned={({data})=>onQr(data)}/><Pressable style={styles.cameraClose} onPress={()=>setCameraOpen(false)}><Text style={styles.primaryText}>Close camera</Text></Pressable></View>:<ScrollView style={{backgroundColor:palette.bg}} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      />}
      {tab==='Settings'&&!demo&&<SettingsScreen data={{name:profileName,email:profile?.email||'',profilePhoto,phoneCountry:profileCountry,phone:profilePhone,currentPassword,newPassword,confirmNewPassword,showConfirmNewPassword,theme,busy,error,notice}} palette={palette} actions={{onChooseProfilePhoto:chooseProfilePhoto,onNameChange:setProfileName,onPhoneCountryChange:setProfileCountry,onPhoneChange:setProfilePhone,onSaveProfile:saveProfile,onCurrentPasswordChange:setCurrentPassword,onNewPasswordChange:setNewPassword,onConfirmPasswordChange:setConfirmNewPassword,onToggleConfirmPassword:()=>setShowConfirmNewPassword(value=>!value),onUpdatePassword:updatePassword,onThemeChange:setTheme}} />}
      {tab==='History'&&!selected&&<HistoryScreen demo={demo} scans={scans} palette={palette} onSelectScan={setSelected} />}
      {selected&&<ScanReportScreen scan={selected} palette={palette} onBack={()=>setSelected(null)} onShare={shareReport} onExportPdf={exportReportPdf} />}
    </ScrollView>}
  </View>;
}
